  ...
  React.useEffect(() => {
    ...
    const finish = async () => {
      try {
          ...
          await signIn.finalize({
            navigate: async ({ session, decorateUrl }) => {
              if (session?.currentTask) {
                setMessage(
                  "This session needs more steps in Clerk. Sending you back to sign-in.",
                );
        ...
        setMessage(
          "Google returned, but Clerk did not finish a session. Start again from sign-in.",
        );
      } catch (error) {
        ...
        setMessage(
          error instanceof Error
            ? error.message
            : "Could not finish sign-in. Start again.",
        );
      }
    };
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-1 justify-center gap-4 px-6">
        ...
        >
          <Text className="text-[15px] font-extrabold text-on-primary-container">
            Back to sign in
          </Text>
        </Pressable>
      </View>
